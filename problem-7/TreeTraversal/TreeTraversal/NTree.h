...
public:
    ...
    {
        // Create a local queue that holds all known nodes
        //  Enqueue first node
        ...
        {
            // Retrieve top of queue and visit
            NTree<T, N> lCurrent = *lQueue.top();
            lQueue.dequeue();
            if (!lCurrent.isEmpty())
                aVisitor.preVisit(lCurrent.key());

            //Foreach of the children, add them to queue
            for (int i = 0; i < N; i++)
            {
                if (!lCurrent[i].isEmpty())
                    lQueue.enqueue(lCurrent.fNodes[i]);
            }
        }
    }
#endif
