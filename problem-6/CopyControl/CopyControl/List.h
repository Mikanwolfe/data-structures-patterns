...
public:
    ...
    {
        // We want to copy the contents of aOtherList to here
        
        // Set up the top and last nodes
        fLast = &Node::NIL;
        fTop = &Node::NIL;
        fCount = 0; // Not necessay but just in case.
                

        // Top vs Last is very confusing. Normally Top is used with Bottom
        //  And First with Last. 
        Node* temp = aOtherList.fTop;

        //Checking for if it's an empty list, if not, copy data, else, nothing.
    ...
    {
        //Override this list with another
        
        if (&aOtherList != this) // Protection against Accidental Suicide
        {
